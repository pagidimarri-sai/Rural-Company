      ...
      <div className="max-w-[1280px] mx-auto px-6">
        ...
        <div className="relative">
          {canScroll && scrollX > 0 && (
            <button
              onClick={scrollLeft}
              className="absolute -left-4 top-1/2 transform -translate-y-1/2 z-10 bg-gray-200 p-2 rounded-full shadow-sm hover:bg-gray-300"
            >
              <FaChevronLeft />
            </button>
          )}
          ...
          {canScroll &&
            ...
            scrollX + scrollRef.current.clientWidth < scrollRef.current.scrollWidth && (
              <button
                onClick={scrollRight}
                className="absolute -right-4 top-1/2 transform -translate-y-1/2 z-10 bg-gray-200 p-2 rounded-full shadow-sm hover:bg-gray-300"
              >
                <FaChevronRight />
              </button>
            )}
          ...
          <div
            ref={scrollRef}
            className="flex gap-5 overflow-x-auto scroll-smooth no-scrollbar w-full pb-2"
          >
            {noteworthyItems.map((item, index) => (
              <div key={index} className="flex flex-col items-center">
                <a
                  href={item.link}
                  className="w-[230px] h-[230px] flex-shrink-0 rounded-xl overflow-hidden hover:scale-105 transition-transform duration-300 shadow-md"
                >
                  <img
                    src={item.imageUrl}
                    alt={item.label}
